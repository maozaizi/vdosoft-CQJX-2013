 ...
 * 类名称：BusDrillingService   
 * 类描述：   
 * 创建人：孟勃婷 
 * 创建时间：Aug 6, 2013 10:04:52 AM
 */
@SuppressWarnings("unchecked")
public class BusDrillingService extends Service {
	 ...
	 */
	public IMap<String,Object> getBusDrillingList(IMap<String,Object> in){
		// 设置结果集
	 ...
	 * 
	 */
	public IMap<String, Object> getBusDrillingDetails(IMap<String, Object> in) {
		IMap<String, Object> result = new DataMap<String, Object>();// 输出map
		IMap selectMap = BeanFactory.getClassBean("com.BusDrilling");
		selectMap.put("id", in.get("id"));
		//进修单信息
		IMap busDrillingMap = db.get(selectMap);
		result.put("busDrillingMap",busDrillingMap );
		//工作流信息
		...
		busInrepairInfoMap.put("id",busDrillingMap.get("inrepairId"));
		busInrepairInfoMap =db.get(busInrepairInfoMap);
		result.put("busInrepairInfoMap",busInrepairInfoMap);
		IMap condition = new DataMap();
		//业务表
		condition.put("busIdOne",busDrillingMap.get("id"));
		condition.put("busNameOne","com.BusDrilling");
		//进修单
		...
		String [] orgIds = busDrillingMap.get("orgId").toString().replace(".",",").split(",");
		//厂区
		...
		baseOrganization.put("orgCode", busDrillingMap.get("orgId"));
	 ...
	 * @return IMap<String,Object>   
	 * @author 孟勃婷
	 * @date Jul 17, 2013 11:07:44 AM
	 */
	public IMap<String,Object> modifyBusDrillingTest(IMap<String,Object> in){
		IMap<String,Object> result = new DataMap<String,Object>();
		IMap selectMap = BeanFactory.getClassBean("com.BusDrilling");
		selectMap.put("id", in.get("id"));
		selectMap= db.get(selectMap);
		selectMap.put("testRemark",in.get("testRemark"));
		IMap userMap = (IMap) in.get("userMap");
		
		selectMap.put("tester",HelperApp.getUserName(userMap));
		Date date=new Date();	
		SimpleDateFormat df=new SimpleDateFormat("yyyy-MM-dd HH:mm:ss");
		String date_str=df.format(date);
		selectMap.put("testDate",date_str);
		//为修改数据赋值
		...
		for(int i=0;i<attachmentInfoList.size();i++){
			...
			attachmentInfoMap.put("busId", selectMap.get("id"));
			...
			attachmentInfoMap.put("busName","com.BusDrilling");
		...
		String remark="厂检："+userMap.get("name")+"在"+dftow.format(new Date())+"添加了设备进厂检验信息。";
		LogBean logb=new LogBean(userMap,in.get("id").toString(), "com.BusDrilling","",remark, "0");
		...
		//wfmap.put("result",(String)in.get("result")); 判断是否检验通过
	...
	public IMap<String, Object> modifyBusMaterialPlan(IMap<String, Object> in) {
		...
		IMap busDrillingMap =BeanFactory.getClassBean("com.BusDrilling");
		busDrillingMap.put("id", busId);
		busDrillingMap.put("isValid",1);
		busDrillingMap = db.get(busDrillingMap);
		//物流计划备注信息
		busDrillingMap.put("planRemark", planRemark);
		db.update(busDrillingMap);
		// 发起工作流
		IMap wfmap = new DataMap();
		IMap condition = new DataMap();
		//获取基础组织编码
		...
		busPlanInfoMap.put("busName","com.BusDrilling");
		...
		if(!busMaterialPlanList.isEmpty()){
			...
			newBusPlanInfoMap.put("busName", "com.BusDrilling");
			...
			//工卡号
			newBusPlanInfoMap.put("workCard",busDrillingMap.get("workCard"));
			//orgId
...
//				}
			...
			LogBean logb=new LogBean(userMap,busId, "com.BusDrilling","",remark, "0");
			LogRecordsService.saveOperationLog(logb,db);
			//auditState - 0  审核不通过 1 审核通过 2未审
...
//			}
	...
	public IMap<String, Object> addBusTestRecord(IMap<String, Object> in) {
		...
		busTestRecord.put("busName","com.BusDrilling");
		//为创建数据赋值
		HelperDB.setCreateInfo(HelperApp.getUserName(userMap),busTestRecord);
		db.insert(busTestRecord);
		SimpleDateFormat dftow=new SimpleDateFormat("yyyy年MM月dd日 HH时mm分");
		String remark="车间检验员："+userMap.get("name")+"在"+dftow.format(new Date())+"添加了试车记录信息。";
		LogBean logb=new LogBean(userMap,busId, "com.BusDrilling","",remark, "0");
		LogRecordsService.saveOperationLog(logb,db);
		//工作流信息
		IMap wfmap = new DataMap();
	...
	public IMap<String, Object> getBusTestRecordDetails(IMap<String, Object> in) {
		...
		busTestRecordMap.put("busName","com.BusDrilling");
		...
		//回写 进修单信息、试车记录
		result.put("busDrillingMap",selectMap);
	 ...
	 */
	public IMap modifyBusDrillingOpinionCheck(IMap in){
		IMap result = new DataMap();
		//试车检验结果
		...
		if("1".equals(checkRes)){
			...
			List workHardMapList = db.getList(workHardMap,null);
			if(!workHardMapList.isEmpty()){
				workHardMap = (IMap)workHardMapList.get(0);
...
//			condition.put("postCode", "调度");
		...
		String remark="检验员："+userMap.get("name")+"在"+sdf.format(new Date())+"对设备进行了出厂检验，意见为："+in.get("opinionChecker")+"；结果为："+resCheck;
		LogBean logb=new LogBean(userMap,(String)in.get("busId"), "com.BusDrilling","",remark, "0");
		LogRecordsService.saveOperationLog(logb,db);
		return result;
	}
	/**
	 ...
	 */
	public IMap modifyBusDrillingComplete(IMap in){
		IMap result = new DataMap();
		IMap selectMap = BeanFactory.getClassBean("com.BusDrilling");
		selectMap.put("id", in.get("busId"));
		//进修单信息
	...
	public IMap doEngineerManage(IMap in){
		...
		factoryMap.put("busName", "com.BusDrilling");
	 ...
	 */
	public IMap modifyBusDrillingReport(IMap in){
		IMap result = new DataMap();
		IMap userMap = (IMap) in.get("userMap");
		List busMaterialPlanList = (List) in.get("busMaterialPlan");
		for (int i = 0; i < busMaterialPlanList.size(); i++) {
		...
		LogBean logb=new LogBean(userMap,(String)in.get("id"), "com.BusDrilling","",remark, "0");
		LogRecordsService.saveOperationLog(logb,db);
	 ...
	 */
	public IMap<String, Object> toConfirmBusDrilling(IMap<String, Object> in) {
		IMap<String, Object> result = new DataMap<String, Object>();// 输出map
		IMap selectMap = BeanFactory.getClassBean("com.BusDrilling");
		selectMap.put("id", in.get("id"));
		//业务表基本信息
		...
		busWorkCardMap.put("busId",busDrillingMap.get("id"));
		busWorkCardMap.put("busName","com.BusDrilling");
		//根据业务表Id的到施工卡列表
	 ...
	 * @date Jul 19, 2013 9:38:03 AM
	 */
	public IMap saveBusDrillingConfirm(IMap in){
		IMap result = new DataMap();
		IMap userMap = (IMap) in.get("userMap");
		// 发起工作流
		IMap wfmap = new DataMap();
		...
		//获取基础组织编码
		IMap busDrillingMap = BeanFactory.getClassBean("com.BusDrilling");
		busDrillingMap.put("id", in.get("id"));
		busDrillingMap = db.get(busDrillingMap);
		busDrillingMap.put("factoryWhere", in.get("factoryWhere"));
		busDrillingMap.put("factoryDate", in.get("factoryDate"));
		
		String rem="";
		String orgId="";
		String [] orgCodes =busDrillingMap.get("orgId").toString().replace(".",",").split(",");
		String postCode="";
		if("1".equals(in.get("result"))){
			...
			factoryMap.put("busId", busDrillingMap.get("id"));
			factoryMap.put("busName", "com.BusDrilling");
			factoryMap.put("affirmType",1);//状态为1调度直接出厂
			factoryMap.put("orgId",userMap.get("orgId"));
			factoryMap.put("workCard",busDrillingMap.get("workCard"));
			factoryMap.put("equipmentName",busDrillingMap.get("equipmentName"));
			factoryMap.put("equipmentModel",busDrillingMap.get("equipmentModel"));
			factoryMap.put("factoryDate", in.get("factoryDate"));
			...
			busRepairSettlementMap.put("workCard",busDrillingMap.get("workCard"));
			busRepairSettlementMap.put("mainCard",busDrillingMap.get("mainCard"));
			busRepairSettlementMap.put("assistCard",busDrillingMap.get("assistCard"));
			HelperDB.setCreate4isValid(HelperApp.getUserName(userMap), busRepairSettlementMap);
			db.insert(busRepairSettlementMap);
			
			busDrillingMap.put("repairState", "4");
			//将进修单信息改为出厂
			IMap inrepairMap = BeanFactory.getClassBean("com.BusInrepairInfo");
			inrepairMap.put("repairState", "4");
			inrepairMap.put("completeDate", in.get("factoryDate"));
			inrepairMap.put("id", busDrillingMap.get("inrepairId"));
		...
		String actionId = "";
		String revenueType = (String)busDrillingMap.get("revenueType");
		...
		//log日志
		SimpleDateFormat sdf=new SimpleDateFormat("yyyy年MM月dd日 HH时mm分");
		String remark="监造："+userMap.get("name")+"在"+sdf.format(new Date())+rem;
		LogBean logb=new LogBean(userMap,(String)in.get("id"), "com.BusDrilling","",remark, "0");
		LogRecordsService.saveOperationLog(logb,db);
	...
	public IMap saveMaterialConfirm(IMap in){
		...
		busDrillingMap.put("id", in.get("id"));
		busDrillingMap = db.get(busDrillingMap);
		String orgId= (String) busDrillingMap.get("orgId");
		String postCode="";
		postCode="车间材料员";
		condition.put("orgId", orgId);
		condition.put("postCode", postCode);
	...
	public IMap<String, Object> toMaterialPlan(IMap<String, Object> in) {
		...
		//当前审批计划
		IMap busPlanInfoMap = BeanFactory.getClassBean("com.BusPlanInfo");
		busPlanInfoMap.put("busId",busDrillingMap.get("id"));
		busPlanInfoMap.put("busName","com.BusDrilling");
		busPlanInfoMap.put("isValid",1);
		//第几份材料申请计划
		...
		condition.put("busNameOne","com.BusDrilling");
		//进修单
		condition.put("busIdTow",busDrillingMap.get("inrepairId"));
		condition.put("busNameTow","com.BusInrepairInfo");
		List<IMap> logRecordsList = db.getList(condition, "getLogRecordsList", "com.logRecords");
		result.put("logRecordsList", logRecordsList);
		//进修单基本信息
	...
	public IMap saveBusWorkCard(IMap in){
		...
		LogBean logb=new LogBean(userMap,in.get("busId").toString(), "com.BusDrilling","",remark, "0");
		LogRecordsService.saveOperationLog(logb,db);
		IMap selectMap = BeanFactory.getClassBean("com.BusDrilling");
		selectMap.put("id", in.get("busId"));
		//进修单信息
		IMap busDrillingMap = db.get(selectMap);
		busDrillingMap.put("dispatchingRemark", in.get("dispatchingRemark"));
		//将开工时间添加到主表中
		busDrillingMap.put("startDate", busWorkCardMap.get("startDate"));
		db.update(busDrillingMap);
		// 发起工作流
		IMap wfmap = new DataMap();
		//wfmap.put("result", flag);// 判断是否受理
	 ...
	 * @date Jul 17, 2013 5:39:18 PM
	 */
	public IMap<String, Object> getbusWorkAndBusDrillingDetails(IMap<String, Object> in) {
		IMap<String, Object> result = new DataMap<String, Object>();// 输出map
		IMap selectMap = BeanFactory.getClassBean("com.BusDrilling");
		selectMap.put("id", in.get("id"));
		//进修单信息
	 ...
	 * @date Jul 17, 2013 5:39:18 PM
	 */
	public IMap<String, Object> getBusDrillingAndMaterialPlanDetails(IMap<String, Object> in) {
		IMap<String, Object> result = new DataMap<String, Object>();// 输出map
		IMap selectMap = BeanFactory.getClassBean("com.BusDrilling");
		selectMap.put("id", in.get("id"));
		//业务表基本信息
		...
		logRecordsMap.put("busId",busDrillingMap.get("id"));
		logRecordsMap.put("busName","com.BusDrilling");
		...
		attachmentMap.put("busId",busDrillingMap.get("id"));
		attachmentMap.put("busName","com.BusDrilling");
		...
		public IMap updateleadershipAudit(IMap in){
			IMap result = new DataMap();
			IMap bussMap = (IMap) in.get("busDrilling");
			IMap userMap = (IMap) in.get("userMap");
			String remark="";
			String docId="";
			String str="";
			...
			IMap busDrillingMap = BeanFactory.getClassBean("com.BusDrilling");
			busDrillingMap.put("id",bussMap.get("id"));
			busDrillingMap = db.get(busDrillingMap);
			//当前审批计划
			IMap busPlanInfoMap = BeanFactory.getClassBean("com.BusPlanInfo");
			busPlanInfoMap.put("busId",busDrillingMap.get("id"));
			...
			if(in.get("type").toString().equals("jl")){
				...
				if(attachmentInfoMap.get("docUrl")!=null){
					attachmentInfoMap.put("busId", busDrillingMap.get("id"));
					attachmentInfoMap.put("id", HelperApp.getAutoIncrementPKID(HelperApp
							.getPackageName(), "com.AttachmentInfo"));
					attachmentInfoMap.put("isValid",Constants.ISVALID);
					attachmentInfoMap.put("busName","com.BusDrilling");
				...
				if("0".equals(bussMap.get("jlResult"))){
					orgId = (String) busDrillingMap.get("orgId");
					postCode="车间材料员";
					spresult = "0";
...
//					}
					...
					//业务信息
					IMap selectMap = BeanFactory.getClassBean("com.BusDrilling");
					selectMap.put("id", busDrillingMap.get("id"));
					selectMap = db.get(selectMap);
					busInrepairInfoMap.put("id", selectMap.get("inrepairId"));
					busInrepairInfoMap = db.get(busInrepairInfoMap);
					//循环发起流程
					for(int i=0;i<materialPlanList.size();i++){
							...
							//判断材料计划中的修理件或者加工件属于哪个车间，直接指派到车间
						...
						//项目部
						machiningMessage.put("proDept", "钻修车间");
						//orgId
				...
				if("0".equals(bussMap.get("zrResult"))){
					...
					addbusworkflow(busPlanInfoMap,busDrillingMap,userMap);
					/**********************************************************************************************************/					
				}
			...
			LogBean logb=null;
			if(!docId.equals("")){
			 logb=new LogBean(userMap,busDrillingMap.get("Id").toString(), "com.BusDrilling","",remark,"1",docId);
			}else{
			 logb=new LogBean(userMap,busDrillingMap.get("Id").toString(), "com.BusDrilling","",remark,"1");
			}
			LogRecordsService.saveOperationLog(logb,db);
		 ...
		 * @date Jul 17, 2013 5:39:18 PM
		 */
		public IMap<String, Object> toBusDrillingDetailsPage(IMap<String, Object> in) {
			IMap<String, Object> result = new DataMap<String, Object>();// 输出map
			IMap selectMap = BeanFactory.getClassBean("com.BusDrilling");
			selectMap.put("id", in.get("id"));
			...
			//业务表基本信息
			IMap busDrillingMap = db.get(selectMap);
			if(busDrillingMap==null){
				IMap selectMaps = BeanFactory.getClassBean("com.BusDrilling");
				selectMaps.put("id", in.get("id"));
				busDrillingMap = db.get(selectMaps);
			}
			//进修单信息
			IMap busInrepairInfo =BeanFactory.getClassBean("com.BusInrepairInfo");
			busInrepairInfo.put("id", busDrillingMap.get("inrepairId"));
			...
			con.put("busId", busDrillingMap.get("id"));
			con.put("busName","com.BusDrilling");
			...
			//试车记录信息
			IMap busTestRecordMap = BeanFactory.getClassBean("com.BusTestRecord");
			busTestRecordMap.put("busId",busDrillingMap.get("id"));
			busTestRecordMap.put("busName","com.BusDrilling");
			OrderBean orderBean = new OrderBean();
	...
	public IMap<String, Object> toInStore(IMap<String, Object> in) {
		...
		//传入条件为机修业务的某一条数据
		//materialMap.put("busId",busDrillingMap.get("id"));
		//materialMap.put("busName","com.BusDrilling");
		//获取附件列表
...
//		}
		...
		for(int i=0;i<outList.size();i++){
			...
			if(storeList.isEmpty()){
				...
				//获取页面填写的出库数量 转换为int
				double outNum = Double.parseDouble((String) tempout.get("storeNum"));
				//循环库存结果，对比数量
				int j = 0;
				int store = 0;
				double amount = 0;
...
//			}
	...
	public IMap<String, Object> toaddNote(IMap<String, Object> in) {
		...
		if (!com.brick.util.Util.checkNull(flag)) {
			...
			IMap<String, Object> busMap = (IMap) in.get("busDrilling");
			...
			result.put("busdrillingMap", busMap);
		}
		return result;
	}
	/**
	...
	public IMap<String, Object> addNote(IMap<String, Object> in) {
		...
		busWorkNote.put("busName", "com.BusDrilling");
		Date date=new Date();	
		SimpleDateFormat df=new SimpleDateFormat("yyyy-MM-dd");
		String date_str=df.format(date);
		
		String busId = (String) busWorkNote.get("busId");
		IMap<String, Object> busMap = BeanFactory.getClassBean("com.BusDrilling");
		busMap.put("id", busId);
	...
	/**
	 * 
	 * @Description 将钻修流程中的加工件以及修理件发送到调度列表，由调度选择修理/加工车间
	 * @param @param in
	 * @param @return    
	 * @return IMap<String,Object>   
	 * @author duch
