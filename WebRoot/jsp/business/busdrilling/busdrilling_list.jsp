...
<script type="text/javascript">
	function doSearch(){
		document.myform.action="${pageContext.request.contextPath}/api/busdrilling/getBusDrillingList";
	...
	function toaddNote(busId){
		window.open("${pageContext.request.contextPath}/api/busdrilling/toaddNote?id="+busId,"","width=700,height=500,scrollbars=no");
	}
</script>
</head>
<body>
			        	...
			        	<thead>
			         	...
			         	<tbody>
				          	...
				          	<c:if test="${not empty page.resultList}">
				            	<c:forEach items="${page.resultList}" var="busDrilling" varStatus="i">
				                	<tr>
				                		 <td>${busDrilling.workcard}</td>
				                		 <td>${busDrilling.deptName }</td>
					                     <td><a href="${pageContext.request.contextPath}/api/busdrilling/toBusDrillingDetailsPage?id=${busDrilling.busId}" target="_blank">${busDrilling.equipmentName}</a></td>
					                     <td>
					                     	<c:if test="${busDrilling.repairType==1}">大修</c:if>
					                   	 	<c:if test="${busDrilling.repairType==2}">中修</c:if>
					                   	 	<c:if test="${busDrilling.repairType==3}">检修</c:if>
					               	 	 </td>
					                     <td>
					                     	<c:if test="${busDrilling.repairProperties==1}">返厂修理</c:if>
					                   	 	<c:if test="${busDrilling.repairProperties==2}">正常修理</c:if>
					                   	 	<c:if test="${busDrilling.repairProperties==3}">返工修理</c:if>
					                   	 </td>
					                     <td><tag:date  value="${busDrilling.inDate}" /></td>
					                     <td>${busDrilling.problem}</td>
					                     <td>
						                     <c:if test="${param.state==1}">
						                     	<a class="btn btn-primary" href="#" onclick="toaddNote('${busDrilling.busId}');"><i class="icon-edit icon-white"></i>生产日报</a>
						                     </c:if>
						                 </td>
				                 	</tr>
				                </c:forEach>
