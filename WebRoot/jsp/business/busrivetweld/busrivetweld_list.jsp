...
<script type="text/javascript">
	function doSearch(){
		document.myform.action="${pageContext.request.contextPath}/api/busrivetweld/getBusRivetweldList";
	...
	function toaddNote(busId){
		window.open("${pageContext.request.contextPath}/api/busrivetweld/toaddNote?id="+busId,"","width=700,height=500,scrollbars=no");
	}
	...
	$(function () {
		$('#myTab a').click(function (e) {
    		...
    		var url = '${pageContext.request.contextPath}/api/busrivetweld/getBusRivetweldList?state='+state;
    	})
	})
</script>
</head>
			        	...
			        	<thead>
			         	...
			         	<tbody>
				          	...
				          	<c:if test="${not empty page.resultList}">
				            	<c:forEach items="${page.resultList}" var="busRivetweld" varStatus="i">
				                	<tr>
				                		 <td>${busRivetweld.workcard}</td>
				                		 <td>${busRivetweld.deptName }</td>
					                     <td><a href="${pageContext.request.contextPath}/api/busrivetweld/toBusRivetweldDetailsPage?id=${busRivetweld.busId}" target="_blank">${busRivetweld.equipmentName}</a></td>
					                     <td>
					                     	<c:if test="${busRivetweld.repairType==1}">大修</c:if>
					                   	 	<c:if test="${busRivetweld.repairType==2}">中修</c:if>
					                   	 	<c:if test="${busRivetweld.repairType==3}">检修</c:if>
					               	 	 </td>
					                     <td>
					                     	<c:if test="${busRivetweld.repairProperties==1}">返厂修理</c:if>
					                   	 	<c:if test="${busRivetweld.repairProperties==2}">正常修理</c:if>
					                   	 	<c:if test="${busRivetweld.repairProperties==3}">返工修理</c:if>
					                   	 </td>
					                     <td><tag:date  value="${busRivetweld.inDate}" /></td>
					                     <td>${busRivetweld.problem}</td>
					                     <td>
						                     <c:if test="${param.state==1}">
						                     	<a class="btn btn-primary" href="#" onclick="toaddNote('${busRivetweld.busId}');"><i class="icon-edit icon-white"></i>生产日报</a>
						                     </c:if>
						                 </td>
				                 	</tr>
				                </c:forEach>
