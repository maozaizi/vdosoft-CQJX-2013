	...
	<head>
		...
		<script type="text/javascript">
			...
			function openMaterial(){
				window.open("${pageContext.request.contextPath}/api/busmechanic/toChooseMaterialInfo","","width=1200,height=700,scrollbars=yes");
			}
			function openMaterialHistory(){
				window.open("${pageContext.request.contextPath}/api/busmechanic/getMaterialListByWorkCard?busName=com.BusRivetweld","","width=800,height=500,scrollbars=yes");
			}
			//删除行
			...
			function savePlan(){
				document.myform.action="${pageContext.request.contextPath}/api/busrivetweld/saveBusMaterialPlan";
				if(checkForm()){
	   ...
	   function openeq(id1,id2){
			window.open("${pageContext.request.contextPath}/api/logRecords/getLogRecordsList?id1="+id1+"&name1=com.BusRivetweld&id2="+id2+"&name2=com.BusInrepairInfo");
	    }
     			...
     			<div class="page-header">
		        	...
		        	<div class="row-fluid">
		                ...
		                <dl class="dl-horizontal">
							...
							<c:if test="${not empty busRivetweldMap.opinionChecker}">
							<dl class="dl-horizontal">
								<dt>
									出厂检验意见：
								</dt>
								<dd>
									${busRivetweldMap.opinionChecker}
								</dd>
							</dl>
							</c:if>
		                </div>
					...
					<h4>领用材料申请</h4>
					 <div class="row-fluid">
					 	<div style="margin:5px 0 5px 5px;"><a class="btn btn-medium" onclick="openMaterial();" href="#"><i class="icon-plus"></i>选择材料</a>&nbsp;&nbsp;<a class="btn btn-medium" onclick="openMaterialHistory();" href="javascript:void(0)"><i class="icon-plus"></i>历史物料申请</a>
					 		材料计划编号：${busRivetweldMap.assistCard}JH${num}
					 		<input class="" type="hidden" maxlength="20" name="planCode" id="planCode" value="${busRivetweldMap.assistCard}JH${num}"/>
					 	</div>
			            	...
			            	<p class="text-right">共<span id="numSpan">${mnum }</span>项材料&nbsp;&nbsp;总计:<input id="price" name="price" class="input-mini" type="text" value="0"  readonly="readonly" /></p>
			            </div>
		  		...
		  		<div class="table">
			  	  <table class="table table-striped table-bordered"> 
	                 	<tr>  
							...
							<th>操作动态<span style="float:right"><a href="javascript:void(0)" onclick="openeq('${busRivetweldMap.id}','${busInrepairInfoMap.id }');">更多>></a></span></th>
							</div>
						</tr>
				  </table>
				  <table class="table table-striped table-bordered">
