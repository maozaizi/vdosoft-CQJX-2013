...
<script type="text/javascript">
	function doSearch(){
		document.myform.action="${pageContext.request.contextPath}/api/businrepairinfo/getTestBusInrepairInfoList";
		document.myform.submit();
	}
	function toCheckBusInrepairInfo() {
		...
		document.myform.action="${pageContext.request.contextPath}/api/businrepairinfo/toCheckBusInrepairInfo";
		document.myform.submit();
	}
    		...
    		<div class="span9">
				...
				<div class="row-fluid">
  				...
  				<!-- 搜索框 end -->
			   	<div class="table">
   					<h4>进修设备列表</h4> 
   					<div class="span10" style="margin-bottom:7px;">
   						<a class="btn btn-primary" href="#" onclick="toCheckBusInrepairInfo();"><i class="icon-edit icon-white"></i>检 验</a>（请选择设备）
          			</div>
			         	...
			         	<tbody>
				          	...
				          	<c:if test="${not empty page.resultList}">
				            	<c:forEach items="${page.resultList}" var="busInrepairInfoMap" varStatus="i">
				                	<tr onclick="selectTr('${busInrepairInfoMap.id }');">
				                	     <td><input type="radio" id="id_${busInrepairInfoMap.id }" name="id" value="${busInrepairInfoMap.id }"/></td>
					                     <td><a href="${pageContext.request.contextPath}/api/businrepairinfo/toBusInrepairInfoPage?id=${busInrepairInfoMap.id}" target="_blank">${busInrepairInfoMap.equipmentName}
					                     </a></td>
					                     <td>
					                     ...
					                     <td>${busInrepairInfoMap.problem}</td>
				                 	</tr>
				                </c:forEach>
			               </c:if>
			         	</tbody>
