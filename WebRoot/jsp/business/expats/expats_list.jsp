...
<script type="text/javascript">
	...
	function update(id) {
		...
		document.myform.action="${pageContext.request.contextPath}/api/expats/toUpdateExpats";
		document.myform.submit();
	}
			         	...
			         	<tbody>
				          	...
				          	<c:if test="${not empty page.resultList}">
				            	<c:forEach items="${page.resultList}" var="expats" varStatus="i">
				                	<tr>
					                     <td><a href="${pageContext.request.contextPath}/api/expats/toDetailExpats?id=${expats.Id}" target="_blank">${expats.taskCode}</a></td> 
					                     <td><tag:date  value="${expats.createDate}" /></td>
