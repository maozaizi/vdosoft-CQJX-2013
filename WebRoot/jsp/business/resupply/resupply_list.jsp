...
<script type="text/javascript">
	function doSearch(){
		document.form.action="${pageContext.request.contextPath}/api/businrepairinfo/getResupplyMainCardList";
	...
	function toUpdate() {
		...
		document.form.action="${pageContext.request.contextPath}/api/businrepairinfo/toResupplyEdit";
		document.form.submit();
	}
</script>
</head>
    		...
    		<div class="span9">
				...
				<div class="row-fluid">
			    	...
			    	<input type="hidden" name="tableName" id="tableName"/>
    	 				<!-- 搜索框开始 start -->
    	 				<fieldset>
            				<legend><span class="label label-info">按条件搜索补充材料信息</span></legend>
				        	<div class="span5">
				        	...
				        	<div class="span10">
				       			...
				       			<a class="btn btn-primary" href="javascript:void(0)" onclick="toUpdate();"><i class="icon-edit icon-white"></i>&nbsp;补充材料</a> 
				       		</div>
				        </fieldset>
				        <!-- 搜索框 结束 end-->
		        	</form>
	                    ...
	                    <tbody>
	                    	...
	                    	<c:if test="${not empty page.resultList}">
		                    	<c:forEach items="${page.resultList}" var="busworkCard" varStatus="i">
			                        <tr>
			                            <td><input type="radio" name="id" value="${busworkCard.bus_id},${busworkCard.tableName}"/></td>
			                            <td>${busworkCard.equipment_name}</td>
			                            <td>${busworkCard.main_card}</td>
			                            <td><c:if test="${busworkCard.repair_type==1}">大修</c:if><c:if test="${busworkCard.repair_type==2}">中修</c:if><c:if test="${busworkCard.repair_type==3}">检修</c:if></td>
