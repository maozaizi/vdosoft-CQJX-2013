...
<script type="text/javascript">
	function doSearch(){
		document.myform.action="${pageContext.request.contextPath}/api/buselectrical/getBusElectricalList";
	...
	function toaddNote(busId){
		window.open("${pageContext.request.contextPath}/api/buselectrical/toaddNote?id="+busId,"","width=700,height=500,scrollbars=no");
	}
			        	...
			        	<thead>
			         	...
			         	<tbody>
				          	...
				          	<c:if test="${not empty page.resultList}">
				            	<c:forEach items="${page.resultList}" var="busElectrical" varStatus="i">
				                	<tr>
				                		 <td>${busElectrical.workcard}</td>
				                		 <td>${busElectrical.deptName }</td>
					                     <td><a href="${pageContext.request.contextPath}/api/buselectrical/toBusElectricalDetailsPage?id=${busElectrical.busId}" target="_blank">${busElectrical.equipmentName}</a></td>
					                     <td>
					                     	<c:if test="${busElectrical.repairType==1}">大修</c:if>
					                   	 	<c:if test="${busElectrical.repairType==2}">中修</c:if>
					                   	 	<c:if test="${busElectrical.repairType==3}">检修</c:if>
					               	 	 </td>
					                     <td>
					                     	<c:if test="${busElectrical.repairProperties==1}">返厂修理</c:if>
					                   	 	<c:if test="${busElectrical.repairProperties==2}">正常修理</c:if>
					                   	 	<c:if test="${busElectrical.repairProperties==3}">返工修理</c:if>
					                   	 </td>
					                     <td><tag:date  value="${busElectrical.inDate}" /></td>
					                     <td>${busElectrical.problem}</td>
					                      <td>
						                     <c:if test="${param.state==1}">
						                     	<a class="btn btn-primary" href="#" onclick="toaddNote('${busElectrical.busId}');"><i class="icon-edit icon-white"></i>生产日报</a>
						                     </c:if>
						                 </td>
				                 	</tr>
				                </c:forEach>
