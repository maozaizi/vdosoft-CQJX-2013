	...
	<head>
		...
		<script type="text/javascript">
			...
			function saveCheck(res){
				...
				document.myform.action="${pageContext.request.contextPath}/api/busmechanic/modifyBusMechanicOpinionCheck";
				if(checkForm()){
					$('#myform').submit();
				}else{
			...
			function checkForm(){
				...
				for(var i=0;i<hours.length;i++){
					if(hours[i].value==""){
						hours[i].style["borderColor"]="red";
						flag=false;
					}else{
						if(!re.test(hours[i].value)){
							hours[i].style["borderColor"]="#C09853";
							flag=false;
						}else{
							hours[i].style["borderColor"]="";
						}
					}
				}
				return flag;
