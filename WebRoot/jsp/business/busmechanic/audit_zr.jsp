...
<html>
	<head>
		...
		function audit(type){
	        $("#zrResult").val(type);      
			document.myform.action="${pageContext.request.contextPath}/api/busmechanic/toZrAudit";
	...
	<div class="container-fluid">
	   <div class="row-fluid">
		    <div class="span9">
		        ...
		        <div class="page-header">
		        	<h4>材料申请</h4>
		        	<p class="text-right">共<input id="numSpan" class="input-mini" type="text" value="0"  readonly="readonly"/></span>项材料&nbsp;&nbsp;总计:<input id="price" name="price" class="input-mini" type="text" value="0"  readonly="readonly" />元</p>
		            ...
		            <div class="page-header">
		        	<h4>加工件</h4>
		        	<c:if test="${fn:length(jiagongPlanList)<6}">
		        	<div class="row-fluid">
		        	</c:if>
		        	<c:if test="${fn:length(jiagongPlanList)>5}">
		        	<div class="row-fluid" style="OVERFLOW-Y: auto; OVERFLOW-X:hidden; height:300px">
		        	</c:if>
		                    <div style="display:block;overflow:auto;height:300px;">
		                    <table class="table table-striped table-bordered" id="">
		                    <tbody>
		                        <c:if test="${empty jiagongPlanList}">
		                            <tr>
		                            	<td colspan="5" style="text-align: center;"><font color='red'>暂无数据！</font></td>
		                            </tr>   
		                        </c:if>
		                        <c:if test="${not empty jiagongPlanList}">
		                            <c:forEach items="${jiagongPlanList}" var="jiagongPlan" >
		                                <tr>
		                                	<c:if test="${empty jiagongPlan.orgid}">
		                                		<input type="hidden" name="jgmaterialId" value="${jiagongPlan.id}"/>
		                                	</c:if>
		                                    <td width="8%">${jiagongPlan.materialGroup}</td>
		                                    <td width="12%">${jiagongPlan.materialCode}</td>
		                                    <td width="25%">${jiagongPlan.materialDetail}</td>
		                       			    <td>
		            ...
		            <div class="page-header">
		        	<h4>修理件</h4>
		        	<c:if test="${fn:length(weixiuPlanList)<6}">
		        	<div class="row-fluid">
		        	</c:if>
		        	<c:if test="${fn:length(weixiuPlanList)>5}">
		        	<div class="row-fluid" style="OVERFLOW-Y: auto; OVERFLOW-X:hidden; height:300px">
		        	</c:if>
		                    <div style="display:block;overflow:auto;height:300px;">
		                    <table class="table table-striped table-bordered" id="">
		                    <tbody>
		                        <c:if test="${empty weixiuPlanList}">
		                            <tr>
		                            	<td colspan="5" style="text-align: center;"><font color='red'>暂无数据！</font></td>
		                            </tr>   
		                        </c:if>
		                        <c:if test="${not empty weixiuPlanList}">
		                            <c:forEach items="${weixiuPlanList}" var="weixiuPlan" >
		                            	<c:if test="${empty jiagongPlan.orgid}">
		                            		<input type="hidden" name="wxmaterialId" value="${weixiuPlan.id}"/>
		                            	</c:if>
		                                <tr>
		                                    <td width="8%">${weixiuPlan.materialGroup}</td>
		                                    <td width="12%">${weixiuPlan.materialCode}</td>
		                                    <td width="25%">${weixiuPlan.materialDetail}</td>
		                                    <td>
